...
namespace Felersnake.Services
{
    public interface ICoordinateChecker
    {
        ...
        bool IsCoordinateImmediatelySafe(Board board, Coordinate toCheck, bool floodCheck = false, bool isTailCheck = false, string myId = "");
    }
    ...
    public class CoordinateChecker : ICoordinateChecker
    {
        ...
        public bool IsCoordinateImmediatelySafe(Board board, Coordinate toCheck, bool floodCheck = false, bool isTailCheck = false, string myId = "")
        {
            ...
            if (board.Snakes.Any(s => s.Body.ToList().Contains(toCheck) && (!s.Body.Last().Equals(toCheck) || (s.Health == 100 && !isTailCheck && !s.Id.Equals(myId)) || floodCheck)))
                return false;
            return true;
        }
    }
